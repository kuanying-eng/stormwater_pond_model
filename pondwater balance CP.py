...
def analyticalResult (C, lam, alpha, nl, ho, linecolor):
    x = np.linspace(0, 1.204)
    y = (C/nl)*np.exp(x*(lam/nl-1/alpha)-lam*ho/nl)
    x1 = np.linspace(1.204,1.3)
    z = -(x1-ho)**b*a/nl
...
''' MAIN '''
lam = 0.24 # mean inflow freq
alpha = 0.15 # mean inflow depth
dt = 0.01       # day 
dyear = 365 * 100    # days 
ho = 1.204 #outlet height
nl = 0.014 #non-outlfow loss
h0 = 1.2 #initial depth
b = 2
C = 313.37 #constant in analytical solution for b = 2
a = 32.958 #outflow coefficient for b = 2
...
Daily_Q = Daily_Q[Daily_Q != 0] # extract the exact outflow events Q>0
